<template
  ><v-container
    fluid
    v-if="songs"
    style="overflow-y: scroll; width: 100%; height: 75vh;  padding: 0 34px 0 0;
    box-sizing: content-box; "
  >
    <v-tabs background-color="white" color="teal accent-4" left>
      <v-tab>{{ title }}</v-tab>
    </v-tabs>
    <v-row justify="center">
      <v-col cols="8" v-for="(item, index) in songs.items" :key="index">
        <v-card
